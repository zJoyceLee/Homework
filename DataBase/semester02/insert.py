...
ss = "INSERT INTO Colleges(name) VALUES "
for name in collegeName:
    sql = ss+"('{0}');".format(name)
    try:
        cursor.execute(sql)
        db.commit()
    except:
...
ss = "INSERT INTO Students(id, name, college_name, passwd) VALUES "
for year in ['12', '13', '14', '15']:
    for i in range(3000):
        ...
        sql = ss+"('{0}', '{1}', '{2}', '{3}');".format(student_id, name, college_name, passwd)
        try:
            cursor.execute(sql)
            db.commit()
        except:
...
ss = "INSERT INTO Teachers(id, name, college_name, passwd) VALUES "
for i in range(100):
    ...
    sql = ss+"('{0}', '{1}', '{2}', '{3}');".format(teacher_id, name, college_name, passwd)
    try:
        cursor.execute(sql)
        db.commit()
    except:
...
ss = "INSERT INTO Courses(id, name, college_name) VALUES "
for i, name in enumerate(CES_className):
    course_id = '008305'+'{0}'.format(i+1).zfill(2)
    sql = ss+"('{0}', '{1}', '计算机工程与科学学院');".format(course_id, name)
    try:
        cursor.execute(sql)
        db.commit()
    except:
...
for i, name in enumerate(EMSD_className):
    ...
    sql = ss+"('{0}', '{1}', '机电工程与自动化学院');".format(course_id, name)
    try:
        cursor.execute(sql)
        db.commit()
    except:
...
for i, name in enumerate(EMSD_className):
    ...
    sql = ss+"('{0}', '{1}', '通信与信息工程学院');".format(course_id, name)
    try:
        cursor.execute(sql)
        db.commit()
    except:
...
for i, name in enumerate(other_className):
    ...
    sql = ss+"('{0}', '{1}', '{2}');".format(course_id, name, random.choice(collegeName[3:]))
    try:
        cursor.execute(sql)
        db.commit()
    except:
