...
from courseSelection.models import S, C, SC
...
def student_course(request):
    ...
    finished_course_list = [sc for sc in SC.objects.all() if sc.sno_id == student.sno and sc.grade and sc.grade > 0]
    selected_course_list = [sc for sc in SC.objects.all() if sc.sno_id == student.sno]
    course_list = C.objects.all()
...
def sc_new(request):
    student = S.objects.get(pk=request.session['role'])
    selected_course_list = [sc.cno for sc in SC.objects.all() if sc.sno_id == student.sno]
...
def auth(request):
    ...
    try:
        student = S.objects.get(logn=username)
    except S.DoesNotExist:
        messages.error(request, u"帐号或密码错误.")
        return redirect(request.META.get('HTTP_REFERER'))
    else:
        if student.pswd == password:
